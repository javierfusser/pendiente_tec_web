...
class Products extends DataBase
{
    ...
    public function edit($jsonOBJ)
    {
        ...
        if (isset($jsonOBJ->id)) {
            ...
            $sql .= "unidades={$jsonOBJ->unidades}, imagen='{$jsonOBJ->imagen}', descripcion='{$descripcion}' WHERE id={$jsonOBJ->id}";
        ...
        if (isset($id)) {
            ...
            if ($result = $this->conexion->query("SELECT * FROM productos WHERE id = {$id}")) {
                ...
                if (!is_null($row)) {
                    ...
                    // Si descripcion tiene JSON, decodificarlo y agregar campos
                    if (isset($row['descripcion']) && !empty($row['descripcion'])) {
                        $desc = json_decode($row['descripcion'], true);
                        if ($desc !== null && is_array($desc)) {
                            // Sobrescribir con los valores del JSON si existen
